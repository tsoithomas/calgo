...
- [ ] 1. Add `CacheConfig` dataclass and wire into `Config`
...
- [ ] 2. Add `get_cache_config()` to `ConfigurationManager`
...
- [ ] 3. Implement `CacheManager`
  - [ ] 3.1 Create `src/cache_manager.py` with `CacheError` exception and `CacheEntry` dataclass (`symbol`, `fetched_at: datetime`, `records: List[PriceData]`)
    - _Requirements: 1.1, 1.4, 2.1_

  - [ ] 3.2 Implement `CacheManager.__init__(self, config: CacheConfig)`
  ...
  - [ ] 3.3 Implement `CacheManager.write(self, symbol: str, records: List[PriceData]) -> Result[None, CacheError]`
  ...
  - [ ] 3.4 Write property test — Property 2: Serialized record field completeness
  ...
  - [ ] 3.5 Write property test — Property 3: Cache file metadata fields
  ...
  - [ ] 3.6 Write property test — Property 4: Cache files written to configured directory
    - **Property 4: After `write()` succeeds, the file shall exist at `{cache_directory}/{SYMBOL}.json`**
    - **Validates: Requirements 1.2, 6.1**

  - [ ] 3.7 Implement `CacheManager.read(self, symbol: str) -> Result[CacheEntry | None, CacheError]`
  ...
  - [ ] 3.8 Write property test — Property 1: Serialization round-trip
    - **Property 1: For any valid `List[PriceData]`, `write()` then `read()` shall return an equivalent list in the same order**
    - **Validates: Requirements 2.5, 7.4**

  - [ ] 3.9 Implement `CacheManager.is_stale(self, entry: CacheEntry) -> bool`
  ...
  - [ ] 3.10 Write property test — Property 5: Staleness detection respects `max_age_days`
...
- [ ] 4. Checkpoint — Ensure all `CacheManager` tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 5. Add cache seeding methods to `CalgoSystem`
  - [ ] 5.1 Instantiate `CacheManager` in `CalgoSystem.initialize()` using `get_cache_config()` and store as `self._cache_manager`
    - _Requirements: 4.1_

  - [ ] 5.2 Implement `CalgoSystem._fetch_and_cache(self, symbol: str) -> CacheEntry | None`
  ...
  - [ ] 5.3 Write property test — Property 9: Lookback date range covers sufficient calendar days
  ...
  - [ ] 5.4 Implement `CalgoSystem._inject_price_history(self, symbol: str, records: List[PriceData]) -> None`
  ...
  - [ ] 5.5 Write property test — Property 8: Seeding injects the correct closing prices
  ...
  - [ ] 5.6 Implement `CalgoSystem._seed_price_history(self, symbols: List[str]) -> None`
  ...
  - [ ] 5.7 Write property test — Property 6: Stale or missing cache triggers a network fetch
  ...
  - [ ] 5.8 Write property test — Property 7: Fresh cache skips network fetch
  ...
  - [ ] 5.9 Call `_seed_price_history()` at the end of the model-setup block in `CalgoSystem.initialize()`, before `transition_to(READY)`
...
- [ ] 6. Update `config/config.json` with optional `cache_config` section
